...
#'
#' @param data A data frame containing the response (y) values, categorical/factor
#'        values (x; numeric or factor), and group values (factor).
#'
#' @param groups The (unquoted) name of a column in \code{data} to serve as
...
#'
#' @param cols A vector of colors to be used for the x variable given in the
...
#'
#' @param type Either "connect", "fitLine" , "anova", "anovaNoLine" or "raw".
#'        \itemize{
...
#'
#' @param \dots Additional arguments to be passed downstream to lattice functions.
#'
#' @return A \code{lattice} plot object is returned invisibly, which must be 
...
rxnNorm <-
  function(formula = NULL, data = NULL, groups = NULL,
    ...
    # Check the input and process the formula & data types
    ...
    if (is.null(groups)) stop("You must provide groups as a factor")
    ...
    panel.connect <- function(x, y, ...) {
      ...
      for (i in i1) {
        if ((method == "sem") | (method == "sem95")) {
          lattice::panel.segments(
            x0 = sumDat[i, 1], y0 = sumDat[i, 3],
            x1 = sumDat[i + 1, 1], y1 = sumDat[i + 1, 3], col = sumDat[i, 5], ...
          )
        }
        if ((method == "mad") | (method == "iqr")) {
          lattice::panel.segments(
            x0 = sumDat[i, 1], y0 = sumDat[i, 4],
    ...
    ### This panel will connect the individual samples across values of a categorical
    ### variable when type = "raw"

    panel.connect2 <- function(x, y, ...) {
      data <- cbind(data, cols)
      f2lvls <- levels(data$fac2)
      nf2 <- length(f2lvls)
      nf1 <- length(levels(data$fac1))
      for (i in 1:nf2) {
        tmp <- data[data$fac2 == f2lvls[i], ]
        for (j in 1:(nf1 - 1)) {
          lattice::panel.segments(
            x0 = tmp[j, "fac1"],
            x1 = tmp[j + 1, "fac1"],
